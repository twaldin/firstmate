...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    omp)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as its own OMP tracked background task, never shell &.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    omp)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/fm-watch-arm.sh OMP tracked background task as directed below.'
      ;;
    *)
