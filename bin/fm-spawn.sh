...
launch_template() {
  ...
  case "$harness" in
    ...
    pi)
      if [ "$kind" = secondmate ]; then
        printf '%s' 'pi __PROVIDERFLAG____MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      else
        printf '%s' 'pi __PROVIDERFLAG____MODELFLAG____EFFORTFLAG__-e __PIEXT__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
