...
That dispatch is bounded only by the always-on lane governor (a default active-worker cap plus swap, RAM, and orphan refusals; `bin/fm-lane-governor.sh`, docs/capacity-failover.md) and any opt-in `config/capacity-failover` host-pressure backpressure that refuses a new spawn.
