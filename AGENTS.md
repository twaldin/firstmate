...
config/neutral-host-drain-path  optional acknowledgement that a home using `bin/fm-supervise-daemon.sh --neutral-host` has another configured path to drain `state/.wake-queue`; LOCAL, gitignored, and not inherited; see docs/configuration.md "Watcher host and away-mode supervisor backend"
