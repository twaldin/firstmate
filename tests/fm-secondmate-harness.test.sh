...
#      config/crew-codex-mcp, and config/backlog-backend - down into each
#      secondmate home's config/, so the secondmate's OWN crewmates, dispatch
#      profiles, Codex MCP posture, and backlog backend inherit the primary's
...
test_propagate_lib() {
  ...
  rm -f "$src/crew-dispatch.json" "$src/crew-harness" "$src/crew-codex-mcp" "$src/backlog-backend"
  propagate_inheritable_config "$src" "$dest"
