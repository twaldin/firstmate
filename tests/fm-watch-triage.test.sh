...
# backstop fail-safe, and away-mode coherence (no double-triage while the
# explicit away-mode daemon owns supervision).
#
...
# --- afk coherence: only the opt-in away daemon gets one-shot watcher triage ---

test_afk_without_daemon_opt_in_keeps_normal_triage() {
  local dir state fakebin out status_file pid
  dir=$(make_case afk-neutral-coherence); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"
  status_file="$state/task.status"
  ...
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  wait_live "$pid" 100 || fail "watcher exited one-shot from .afk without explicit daemon opt-in"
  [ ! -s "$state/.wake-queue" ] || { reap "$pid"; fail "neutral watcher enqueued a benign signal because .afk existed"; }
  reap "$pid"
  pass ".afk alone does not switch the watcher into daemon-owned one-shot mode"
}

test_daemon_away_opt_in_reverts_watcher_to_one_shot() {
  local dir state fakebin out drain_out status_file pid
  ...
  printf 'working: routine note\n' > "$status_file"
  date '+%s' > "$state/.afk"
  # Set a PROVABLY-WORKING verdict: if the daemon-owned away flag failed to
  # bypass the provably-working check, this no-verb signal would be absorbed.
  # The test asserting a surface therefore also proves one-shot mode skips the
  # costly read.
  export FM_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
  watch_bg "$state" "$fakebin" "$out" env FM_WATCHER_AWAY_MODE=1
  pid=$!
  wait_for_exit "$pid" 100 || fail "with explicit away opt-in the watcher did not exit one-shot for a benign signal"
  grep -F "signal: $status_file" "$out" >/dev/null || fail "away-mode watcher did not surface the signal for the daemon"
  FM_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the afk-mode signal failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
    || fail "away-mode benign signal was not queued for the daemon to classify"
  pass "explicit away opt-in reverts the watcher to one-shot so the daemon owns triage"
}
