...
3. First cycle: arm with OMP's tracked background tool, as its own call:
...
OMP should surface a background-task-completed reminder when the background arm completes.
When you see a background-task-completed system reminder for the arm:
1. Run `bin/fm-wake-drain.sh` first.
2. Optionally fetch arm output with the harness's background-output command for the reason line.
...
Do not run the primary firstmate as a one-shot headless OMP process.
