...
| `fm-supervise-daemon.sh` | Neutral watcher host by default: respawn `fm-watch.sh` with daemon lock/pid/backoff/beacon machinery while leaving wake queueing to the watcher; `--away-mode` opts into the `/afk` classifier, housekeeping, tmux/herdr injection, batched digests, and delivery alarms |
