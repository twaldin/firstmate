...
A neutral host with an empty queue is allowed only when `config/neutral-host-drain-path` exists to acknowledge another queue-drain path, while a `mode=away` daemon without the `.afk` presence gate also blocks in-flight work because away-mode ownership itself is incoherent.
